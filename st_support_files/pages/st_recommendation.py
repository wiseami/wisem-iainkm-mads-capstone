...
def write():
    ...
    with st.container():
        ...
        try:
            ...
            tops = clusters_by_country[clusters_by_country['basic_kmeans_cluster']==final_df['basic_kmeans_cluster'].item()].sort_values(by='track_id', ascending=False)[0:1]
            ...
            cossim_df = cossim_df.drop(columns=['update_dttm', 'time_signature', 'name','artist','album_img','preview_url', 'basic_kmeans_cluster', 'popularity', 'pl_count', 'adv_kmeans_cluster'])
            cossim_df_y = cossim_df['track_id']
            cossim_df = cossim_df.drop(columns=['track_id','tempo_1','tempo_2','duration_ms'])

            compare_df = final_df.copy()
            compare_df = compare_df.drop(columns=['update_dttm', 'time_signature', 'name','artist','album_img','preview_url', 'basic_kmeans_cluster', 'popularity'])
            compare_df_y = compare_df['track_id']
            compare_df = compare_df.drop(columns=['track_id','tempo','duration_ms'])
            ...
            if model_type == 'basic':
                ...
                cossim_df_f['sim'] = cossim_df_f.apply(lambda x: cosine_similarity(compare_df.values.reshape(1,-1), x.values.reshape(1,-1))[0][0], axis=1)
                cossim_df_f['track_id'] = cossim_df_y
                cossim_df_f = cossim_df_f[cossim_df_f['sim'] < 1]
                cossim_df_sort = cossim_df_f.sort_values('sim',ascending=False)[0:5]
                ...
                compare_df = compare_df.merge(final_df[['track_id','name','artist','album_img','preview_url']], how='inner', on='track_id')
