...
def write():
    ...
    with st.container():
            ...
            with st.spinner(text='Recommendizer running...'):
                for item in search['artists']['items'][0:1]:
                    ...
                    for top_tracks in searchy['tracks'][0:5]:
                        if st.button(top_tracks['name'], key ='artist'):
        ...
        #     search_term = 'all too well'
        #     search = requests.get(SPOTIFY_BASE_URL + 'search?q=track:' + search_term + '&type=track', headers=headers)
        #     search = search.json()
        ...
        #             list_items.append((item['artists'][0]['name'] + ' - ' + item['name']))
        #         count = 1
        #         for item in list_items:
        #             count+=1
        #             if st.button(item, key =count):
